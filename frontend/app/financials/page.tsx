...
const TABS = [
  ...
  { key: "loan",        label: "Loan & EMI",      icon: CreditCard },
  ...
  return (
    <div className="animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        ...
        <SummaryCard label="Monthly EMI" value={fmtFull(data.emi)} sub={`${data.tenure_months} months`} icon={Clock} color="text-[#ea580c]" />
        ...
        <SummaryCard label="Annual ROI" value={`${data.roi}%`} sub="Return on investment" icon={BarChart3} color={data.roi >= 15 ? "text-[#16a34a]" : "text-[#ea580c]"} />
      </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start justify-between gap-6">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-sm font-bold text-[#fde047]">Eligible Govt. Scheme</p>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl p-5 border border-premium-border shadow-sm">
          <p className="text-xs font-bold text-ink-soft uppercase mb-1">DSCR (Repayment Capacity)</p>
      ...
      <div className="bg-white rounded-2xl border border-premium-border p-6 mb-6">
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={cf} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
            ...
            <Tooltip
              formatter={(val: any, name: string) => [fmtFull(val), name]}
...
const LoanTab = ({ data }: { data: FinanceResponse }) => {
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <h2 className="text-xl font-bold text-forest-deep mb-6">Loan & EMI Details</h2>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <SummaryCard label="Monthly EMI" value={fmtFull(data.emi)} sub={`For ${data.tenure_months - data.moratorium_months} months`} icon={Clock} color="text-ink" />
      ...
      <div className="bg-white rounded-2xl border border-premium-border shadow-sm overflow-hidden">
        <div className="px-6 py-4 bg-[#f9f8f5] border-b border-premium-border">
          <h3 className="font-bold text-forest-deep">EMI Amortization (First 6 Months)</h3>
        </div>
        ...
        <div className="px-6 py-3 text-xs text-ink-soft bg-[#f9f8f5] border-t border-premium-border">
          {data.moratorium_months > 0 && `⏸ Moratorium period: ${data.moratorium_months} months (no EMI). `}
          EMI payments start from Month {data.moratorium_months + 1}.
        </div>
      </div>
    </div>
  );
          ...
          return (
            <div key={key} className={`${bg} border ${border} rounded-2xl p-6`}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  <span className="text-xs text-ink-soft">ROI</span>
...
export default function FinancialsPage() {
  const { sessionId, categoryId } = useStore();
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#fcfbf8] flex flex-col items-center justify-center">
        ...
        <h2 className="text-lg font-bold text-ink">Building your financial model...</h2>
        <p className="text-sm text-ink-soft">Running projections, EMI and scenario analysis</p>
      </div>
    );
  }
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-[#fcfbf8] flex flex-col items-center justify-center p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-ink mb-2">Financial Analysis Failed</h2>
        <p className="text-ink-soft mb-6">{error || "No session data found. Please complete onboarding first."}</p>
